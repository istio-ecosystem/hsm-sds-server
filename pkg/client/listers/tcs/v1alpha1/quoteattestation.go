...
type QuoteAttestationLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.QuoteAttestation, err error)
...
// List lists all QuoteAttestations in the indexer.
func (s *quoteAttestationLister) List(selector labels.Selector) (ret []*v1alpha1.QuoteAttestation, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.QuoteAttestation))
	})
	return ret, err
}
...
type QuoteAttestationNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha1.QuoteAttestation, error)
...
// List lists all QuoteAttestations in the indexer for a given namespace.
func (s quoteAttestationNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.QuoteAttestation, err error) {
...
// Get retrieves the QuoteAttestation from the indexer for a given namespace and name.
func (s quoteAttestationNamespaceLister) Get(name string) (*v1alpha1.QuoteAttestation, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("quoteattestation"), name)
	}
	return obj.(*v1alpha1.QuoteAttestation), nil
}
